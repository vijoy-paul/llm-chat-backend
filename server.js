...
app.post('/api/chat', async (req, res) => {
    ...
    try {
        ...
        // Prepend system prompt for friendly LLM persona
        const systemPrompt = {
            role: 'system',
        ...
        // Enforce 1000 char limit on latest user message
        if (lastMsg && lastMsg.content && lastMsg.content.length > 1000) {
            return res.status(400).json({ error: "Message too long (max 1000 characters)." });
        }
        // Try a list of models from env, in order
